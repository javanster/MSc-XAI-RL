...
def eval_ice_concepts_ccm_dt(layer_i: int, c: int):
    ...
    with tqdm(total=len(target_types) * len(max_depths), unit="score") as pbar:
        for target_type, max_depth in itertools.product(target_types, max_depths):
            ...
            cavs = np.load(
                f"rl_ice_data/ncavs/gold_run_mini/model_of_interest_target_class_balanced_observations/layer_{layer_i}/c_{c}_ncavs.npy"
            )
...
if __name__ == "__main__":
    eval_ice_concepts_ccm_dt(layer_i=1, c=47)  # Based on best layer of nn ccm
