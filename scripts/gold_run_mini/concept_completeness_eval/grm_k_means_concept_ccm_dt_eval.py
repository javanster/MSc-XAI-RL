...
def eval_k_means_concepts_ccm_dt(layer_i: int, k: int):
    ...
    with tqdm(total=len(target_types) * len(max_depths), unit="score") as pbar:
        for target_type, max_depth in itertools.product(target_types, max_depths):
            ...
            cavs = np.load(
                f"rl_ace_data/concept_examples/k_means/gold_run_mini/model_of_interest_target_class_balanced_observations/layer_{layer_i}/k_{k}_cluster_centroids.npy"
            )
            ...
            cluster_labels = np.load(
                f"rl_ace_data/concept_examples/k_means/gold_run_mini/model_of_interest_target_class_balanced_observations/layer_{layer_i}/k_{k}_cluster_labels.npy"
            )
            biases = []
...
if __name__ == "__main__":
    eval_k_means_concepts_ccm_dt(layer_i=4, k=50)  # Based on best layer of nn ccm
