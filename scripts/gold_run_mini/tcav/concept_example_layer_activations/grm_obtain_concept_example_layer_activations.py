        ...
        for layer_i in range(len(model.layers)):
            for approach in approach_datasets.keys():
                ...
                for batch_i, observation_batch in enumerate(observation_batches):
                    ...
                    dir_path = f"/Volumes/MemoryBrick/MSc/rl_tcav_data/concept_example_layer_activations/gold_run_mini/model_{MODEL_OF_INTEREST_NAME}/{approach}/concept_{concept_name}/layer_{layer_i}/"
...
if __name__ == "__main__":
    ...
    pos_gold_above_datasets = load_numpy_arrays_from_paths(
        paths=CONCEPT_EXAMPLE_PATHS,
        array_filename_prefix="binary_concept_gold_above",
        array_filename_ending="positive_examples",
    )

    neg_gold_above_datasets = load_numpy_arrays_from_paths(
        paths=CONCEPT_EXAMPLE_PATHS,
        array_filename_prefix="binary_concept_gold_above",
    ...
    get_and_save_concept_example_activations(
        approach_datasets=pos_gold_above_datasets, model=model, concept_name="gold_above_positive"
    )
    get_and_save_concept_example_activations(
        approach_datasets=neg_gold_above_datasets, model=model, concept_name="gold_above_negative"
    )
    get_and_save_concept_example_activations(
