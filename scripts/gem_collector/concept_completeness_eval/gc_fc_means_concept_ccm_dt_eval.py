    ...
    with tqdm(total=len(target_types) * len(max_depths), unit="score") as pbar:
        for target_type, max_depth in itertools.product(target_types, max_depths):
            ...
            cluster_labels = np.load(
                f"rl_ace_data/concept_examples/fuzzy_c_means/gem_collector/model_of_interest_target_class_balanced_observations/layer_{layer_i}/c_{c}_hard_cluster_labels.npy"
            )
            biases = []
...
if __name__ == "__main__":
    eval_fc_means_concepts_ccm_dt(layer_i=5, c=20)  # Based on best layer of nn ccm
