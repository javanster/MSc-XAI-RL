...
def eval_fc_means_concepts_ccm_dt():
    ...
    X_TEMPLATE = (
        "/Volumes/work/rl_ccm_data/obs_action_set/gem_collector/denim-sweep-56/"
        "batch_{batch}/{file}"
    )
    FC_BASE = (
        "/Volumes/work/rl_ace_data/concept_examples/fc_means/"
        "gem_collector/model_of_interest_target_class_balanced_observations"
    )
    observations = np.load(
        "/Volumes/work/rl_concept_discovery_data/class_datasets_model_of_interest/"
        "gem_collector/target_class_balanced_30000_shuffled_examples.npy"
    )
    ...
    for batch in batches:
        X_train = np.load(X_TEMPLATE.format(batch=batch, file="X_train_8000_examples.npy"))
        X_val = np.load(X_TEMPLATE.format(batch=batch, file="X_val_2000_examples.npy"))
        Y_train = np.load(
            X_TEMPLATE.format(batch=batch, file=f"Y_{target_type}_train_8000_examples.npy")
        )
        Y_val = np.load(
            X_TEMPLATE.format(batch=batch, file=f"Y_{target_type}_val_2000_examples.npy")
        )
            ...
            for c in cs_per[(batch, layer_i)]:
                ...
                cluster_labels = np.load(
                    os.path.join(
                        ...
                        f"batch_{batch}",
                        f"layer_{layer_i}",
                        f"c_{c}_hard_cluster_labels.npy",
                    )
                )
                ...
                for cav_idx, cav in enumerate(cavs):
                    ...
                    if obs_pos.shape[0] == 0:
                        # no real positives - fake one from the CAV itself
                        pos_act = cav[None, :]
                    else:
                        pos_act = mao.get_layer_activations(
                            layer_index=layer_i,
                            model_inputs=obs_pos,
                            flatten=True,
                        )
