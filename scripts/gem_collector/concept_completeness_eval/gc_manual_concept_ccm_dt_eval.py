...
def eval_manual_concepts_ccm_dt():
    ...
    with tqdm(total=len(layers), unit="layer") as pbar:
        for layer_i in layers:
            ...
            all_biases = []
            all_names = []
            for fn in sorted(os.listdir(concept_probe_path)):
                if not fn.endswith(f"_layer_{layer_i}_concept_probe.keras"):
                    continue
                name = fn[: fn.rfind(f"_layer_{layer_i}_concept_probe.keras")]
                probe: Sequential = load_model(os.path.join(concept_probe_path, fn))  # type: ignore
            ...
            ccm_all = CCM_DT(
                ...
                X_val=X_val,
                Y_train=np.load(
                    "rl_ccm_data/obs_action_set/gem_collector/denim-sweep-56/"
            ...
            score_all, dt_model_all = ccm_all.train_and_eval_ccm(
                cavs=all_cavs,
            ...
            cavs_filt, names_filt = get_manual_cavs_and_names(
                manual_concept_probe_path=concept_probe_path,
                layer_i=layer_i,
            ...
            for name in names_filt:
                ...
                probe = load_model(os.path.join(concept_probe_path, fn))  # type: ignore
            ...
            score_filt, dt_model_filt = ccm_filt.train_and_eval_ccm(
                cavs=cavs_filt,
                conv_handling="flatten",
                layer_i=layer_i,
