...
def eval_fc_means_concepts_ccm_dt(layer_i: int, c: int):
    ...
    cavs = np.load(
        f"rl_ace_data/concept_examples/fuzzy_c_means/minecart_counter/model_of_interest_target_class_balanced_observations/layer_{layer_i}/c_{c}_cluster_centroids.npy"
    )
    ...
    cluster_labels = np.load(
        f"rl_ace_data/concept_examples/fuzzy_c_means/minecart_counter/model_of_interest_target_class_balanced_observations/layer_{layer_i}/c_{c}_hard_cluster_labels.npy"
    )
    biases = []
    for cav_idx, cav in enumerate(cavs):
        ...
        if len(observations_pos) == 0 or len(observations_neg) == 0:
            biases.append(0.0)
            continue
    ...
    with tqdm(total=len(target_types) * len(max_depths), unit="score") as pbar:
        for target_type, max_depth in itertools.product(target_types, max_depths):
            ...
            joblib.dump(
                model,
                f"{save_path}best_ccm_fc_means_layer_{layer_i}_c_{c}_{target_type}_max_depth_{max_depth}.joblib",
            )

            pbar.update(1)

    df = pd.DataFrame(results)
    df.to_csv(f"{save_path}fc_means_concepts_completeness_scores.csv", index=False)


if __name__ == "__main__":
    eval_fc_means_concepts_ccm_dt(layer_i=5, c=25)  # Based on best layer of nn ccm
