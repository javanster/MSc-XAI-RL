...
class GoldRun(Env):
    """
    A Gymnasium environment where the goal of the agent is to get through three rooms.

    Each room has added complexity. In the first room, the agent only has to reach the passage and
    enter through it. In the second, the agent has to get to the second passage while avoiding
    lava. In the last room, the agent still has to avoid lava, but the passage is closed, and is
    only opened once the agent has collected two pieces of gold.
    """
    ...
    def __init__(
        ...
        self.VALID_PASSAGE_POSITIONS: List[Tuple[int, int]] = [(7, 0), (14, 7), (7, 14), (0, 7)]

        self.STEP_PENALTY = -0.0025
        self.LAVA_PENALTY = -0.5
        self.GOLD_REWARD = 0.14375
        self.PASSAGE_REWARD = 0.26
    ...
    def _get_new_agent_pos(self):
        ...
        return opposite_mapping.get(self.current_passage_position, None)

    def _next_room(self, room_n: int):
        if room_n > 3:
            raise ValueError("There are no more rooms than 3")
    ...
    def reset(
        ...
        self.passage: Entity | None = Entity(
            grid_side_length=self.grid_side_length,
            color=self.OPEN_PASSAGE_COLOR,
            starting_position=self.current_passage_position,
        )

        for wall in self.walls[:]:
            if self.passage and wall == self.passage:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[Any, Any]]:
        ...
        if not self._is_passage_open() and self.agent == self.passage:
            self.agent.x = original_agent_x
            self.agent.y = original_agent_y
    ...
    def _draw_sprites(self, canvas: pygame.Surface, pix_square_size: float) -> None:
        ...
        for wall in corner_walls:
            rotation_degrees = 0
            if wall.x == 14 and wall.y == 0:
                rotation_degrees = 180
            elif wall.x == 14 and wall.y == 14:
                rotation_degrees = 90
        ...
        if self.passage:
            ...
            if self.passage.x == 0:
                rotation_degrees = 90
            elif self.passage.y == 14:
                rotation_degrees = 180
            elif self.passage.x == 14:
                rotation_degrees = 270
